...
char *clib_binary_heap_get_as_string(clib_binary_heap_t *heap)
{
    ...
    char *ret = NULL;
    char tmp[12] = {0};
    size_of_string += clib_math_int_width(heap->tree.array[0].key, CLIB_RADIX_DEC); /* "num" */
    for (i = 1; i < heap->tree.size - 1; i++)
    {
        size_of_string += clib_math_int_width(heap->tree.array[i].key, CLIB_RADIX_DEC) + 2; /* "num, " */
    }
    if (heap->tree.size >= 3)
    {
        size_of_string += clib_math_int_width(heap->tree.array[heap->tree.size - 1].key, CLIB_RADIX_DEC); /* "num" */
    }
    ret = calloc(size_of_string, sizeof(char));
    ret[0] = '[';
    ret[1] = ' ';
    clib_string_from_int(tmp, heap->tree.array[0].key, CLIB_RADIX_DEC);
    strcat(ret, tmp);
    strcat(ret, ", ");
    for (i = 1; i < heap->tree.size - 1; i++)
    {
        clib_string_from_int(tmp, heap->tree.array[i].key, CLIB_RADIX_DEC);
        strcat(ret, tmp);
        strcat(ret, ", ");
    }
    clib_string_from_int(tmp, heap->tree.array[heap->tree.size - 1].key, CLIB_RADIX_DEC);
    strcat(ret, tmp);
    strcat(ret, " ]");
    return ret;
}
