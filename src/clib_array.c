...
clib_array_t *clib_array_push(clib_array_t *array, const void *src)
{
    ...
    clib_memory_copy(src, (uint8_t *)array->buf + (array->type_size * array_index), array->type_size);
    array->length++;
    return array;
}
...
clib_array_t *clib_array_get(clib_array_t *array, size_t index, void *dest)
{
    ...
    clib_memory_copy(dest, (uint8_t *)array->buf + (index * array->type_size), array->type_size);
    return array;
}
