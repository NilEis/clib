    ...
    for leaf in structs:
        for subleaf in structs[leaf]:
            ...
            for reg in structs[leaf][subleaf].dict:
                out_file.write(TABSIZE+"union\n")
                out_file.write(TABSIZE+"{\n")
                out_file.write(TABSIZE+TABSIZE+"struct\n")
                out_file.write(TABSIZE+TABSIZE+"{\n")
                unused_counter = 0
                for elem in structs[leaf][subleaf].dict[reg]:
                    if not int(elem.bit_start) == int(elem.last_bit):
                        out_file.write(TABSIZE+TABSIZE+TABSIZE+"uint32_t unused_"+str(unused_counter)+": "+str(int(elem.bit_start)-int(elem.last_bit))+"; /* unused */\n")    
                        unused_counter = unused_counter + 1
                    out_file.write(TABSIZE+TABSIZE+TABSIZE+"uint32_t "+elem.name+": "+elem.bits+"; /* "+elem.comment+" */\n")

                out_file.write(TABSIZE+TABSIZE+"} field;\n")
                out_file.write(TABSIZE+TABSIZE+"uint32_t value;\n")
    ...
    for leaf in structs:
        ...
        for subleaf in structs[leaf]:
            ...
            for reg in elem.dict:
                out_src_file.write(TABSIZE+TABSIZE+TABSIZE+name+"."+reg+".value = "+reg+";\n")
